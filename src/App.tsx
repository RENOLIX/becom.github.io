  ...
  return (
    <main className="product-page shell">
      ...
      <div className="product-detail">
        ...
        <div className="detail-copy"><div className="detail-top"><span className="product-badge static">{product.badge || "Sélection BECOM"}</span><button className="icon-button"><Heart /></button></div><span className="eyebrow">{product.category} · {product.age}</span><h1>{productName(product, isArabic)}</h1><div className="rating"><span><Star fill="currentColor" /> {product.rating}</span></div><p className="detail-description">{productDescription(product, isArabic)}</p><div className="skill-list">{product.skills.map((skill) => <span key={skill}><Sparkles /> {skill}</span>)}</div>{product.showColor && availableColors.length > 0 && <div className="color-choice"><p>Choisir la couleur</p><div>{availableColors.map((color) => <button type="button" className={activeColor === color ? "active" : ""} key={color} onClick={() => setSelectedColor(color)}><i style={{ background: resolveProductColor(color) }} />{color}</button>)}</div></div>}{availablePieces.length > 0 && <div className="piece-choice"><p>Choisir une option</p><div>{availablePieces.map((option) => <button type="button" className={selectedPieceKey === pieceKey(option) ? "active" : ""} key={pieceKey(option)} onClick={() => { setSelectedPieceKey(pieceKey(option)); setOptionError(""); }}><span><strong>{option.pieces}</strong> pièces</span><b>{option.name}</b><em>{money(option.price)}</em></button>)}</div>{optionError && <small>{optionError}</small>}</div>}<div className="detail-price" aria-live="polite"><strong key={displayPrice}>{money(displayPrice)}</strong>{product.oldPrice && !selectedPiece && <del>{money(product.oldPrice)}</del>}</div><div className="stock"><i /> En stock · expédition sous 24/48h</div><div className="purchase-row"><div className="quantity"><button type="button" aria-label="Diminuer la quantité" onClick={() => setQuantity((current) => Math.max(1, current - 1))}><Minus /></button><input className="quantity-value" value={quantity} readOnly aria-label="Quantité sélectionnée" /><button type="button" aria-label="Augmenter la quantité" onClick={() => setQuantity((current) => current + 1)}><Plus /></button></div><button type="button" className="button primary purchase" onClick={addToCart}>Ajouter au panier <ShoppingBag /></button></div><div className="detail-assurances"><div><Truck /><span><strong>Livraison rapide</strong><small>À partir de 500 DA</small></span></div><div><ShieldCheck /><span><strong>Paiement sécurisé</strong><small>Ou à la livraison</small></span></div><div><Gift /><span><strong>Option cadeau</strong><small>Message personnalisé</small></span></div></div></div>
      </div>
